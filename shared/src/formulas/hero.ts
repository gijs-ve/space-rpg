import { BASE_MAX_ENERGY, BASE_MAX_HEALTH, BASE_ENERGY_REGEN, BASE_HEALTH_REGEN, ENERGY_HEALTH_PER_LEVEL, SKILLS, SkillId } from '../constants/skills';
...
/**
 * Maximum energy for a hero based on hero level.
 * Each level after 1 grants +ENERGY_HEALTH_PER_LEVEL max energy.
 */
export function computeMaxEnergy(heroLevel: number): number {
  return BASE_MAX_ENERGY + (heroLevel - 1) * ENERGY_HEALTH_PER_LEVEL;
}

/**
 * Maximum health for a hero based on hero level.
 * Each level after 1 grants +ENERGY_HEALTH_PER_LEVEL max health.
 */
export function computeMaxHealth(heroLevel: number): number {
  return BASE_MAX_HEALTH + (heroLevel - 1) * ENERGY_HEALTH_PER_LEVEL;
}

/**
 * Energy regenerated by a hero per global regen tick (every 5 minutes).
 * Base regen plus endurance skill bonus plus item bonus.
 */
export function computeEnergyRegenPerTick(
  skillLevels: SkillLevels,
  itemBonuses: ItemBonus = {}
): number {
  const enduranceLevel = skillLevels.endurance ?? 0;
  const skillBonus = enduranceLevel * (SKILLS.endurance.bonusPerLevel['energyRegenBonus'] ?? 0);
  const itemBonus  = itemBonuses.energyRegenBonus ?? 0;
  return BASE_ENERGY_REGEN + skillBonus + itemBonus;
}

/**
 * Health regenerated by a hero per global regen tick (every 5 minutes).
 * Base regen plus item bonus.
 */
export function computeHealthRegenPerTick(
  _skillLevels: SkillLevels,
  itemBonuses: ItemBonus = {}
): number {
  const itemBonus = itemBonuses.healthRegenBonus ?? 0;
  return BASE_HEALTH_REGEN + itemBonus;
}
...
export interface ComputedHeroStats {
  ...
  defense: number;
  /** Max energy — level-based: BASE_MAX_ENERGY + (level-1) * ENERGY_HEALTH_PER_LEVEL */
  maxEnergy: number;
  /** Max health — level-based: BASE_MAX_HEALTH + (level-1) * ENERGY_HEALTH_PER_LEVEL */
  maxHealth: number;
  /** Energy regenerated per global tick (endurance skill + items) */
  energyRegenPerTick: number;
  /** Health regenerated per global tick (items only for now) */
...
export function computeHeroStats(
  ...
  const maxEnergy = computeMaxEnergy(heroLevel);
  const maxHealth = computeMaxHealth(heroLevel);
  const energyRegenPerTick = computeEnergyRegenPerTick(skillLevels, itemBonuses);
  const healthRegenPerTick = computeHealthRegenPerTick(skillLevels, itemBonuses);
  ...
  return { attack, defense, maxEnergy, maxHealth, energyRegenPerTick, healthRegenPerTick, gatheringBonus, adventureSpeedReduction };
}
